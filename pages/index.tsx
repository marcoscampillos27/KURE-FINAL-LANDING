...
function looksLikeEmail(email: string) { return EMAIL_REGEX.test(email); }
function isDisposable(email: string) {
  const d = email.split('@')[1]?.toLowerCase();
  return new Set(['mailinator.com','tempmail.com','10minutemail.com','guerrillamail.com','yopmail.com','sharklasers.com','trashmail.com','trashmail.de','getnada.com']).has(d || '');
}
...
export default function Home() {
  ...
  useEffect(() => {
    if (!msg && !err) return;
    const t = setTimeout(() => { setMsg(null); setErr(null); }, 8000);
    return () => clearTimeout(t);
  }, [msg, err]);

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setMsg(null); setErr(null); setDownloadReady(false);
    ...
    try {
      ...
      setMsg('Descarga lista. Hemos guardado tu email.');
        ...
        <div className="rounded-2xl border p-6 bg-white">
          ...
          <form onSubmit={handleSubmit} className="mt-6 grid gap-4 md:grid-cols-[1fr_auto]">
            <input type="email" required placeholder="tu@email.com"
              className="w-full rounded-xl border px-4 py-3 focus:ring-2 focus:ring-blue-300 outline-none"
              value={email} onChange={(e)=>setEmail(e.target.value)} />
            <button disabled={loading} className="rounded-xl bg-blue-600 px-6 py-3 text-white disabled:opacity-60">
              {loading ? 'Procesando…' : 'Obtener guía'}
            </button>
          </form>
          ...
          {downloadReady && (
            <div className="mt-6 border p-4 bg-emerald-50 rounded-xl">
              <p className="font-medium">Tu guía está lista ✅</p>
              <button onClick={handleDownload} className="mt-3 rounded-lg bg-emerald-600 px-4 py-2 text-white">
                Descargar Guía
              </button>
            </div>
          )}
